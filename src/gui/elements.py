...
class RhombusButton(Element):
    def __init__(self, scene: Scene, pos: _pair, text: str, font: _font, color: _color, command: Callable, key: int = 0, anchor: Anchors = Anchors.CENTER) -> None:
        ...
        self.tween_shrink.linear_val = self.tween_shrink.start
    ...
    def update(self) -> None:
        if not self.locked:
            try:
                ...
                if self.mask.get_at(inttup(pygame.mouse.get_pos() - anchored(self.anchor_pos, VEC(self.mask.get_size()), self.anchor))): # If the position of the cursor is part of the mask
                    self.hover()
...
class RectButton(Element):
    def __init__(self, scene: Scene, pos: _pair, text: str, font: _font, color: _color, command: Callable, key: int = 0, anchor: Anchors = Anchors.CENTER) -> None:
        ...
        self.default_factor, self.hover_factor = 1.5, 1.8
        self.factor = self.line_factor = self.default_factor
        self.surface, self.mask = self.default_surf, self.default_mask = self.generate_image(self.factor, False)
        self.hover_surf, self.hover_mask = self.generate_image(self.hover_factor, True)
    ...
    def update(self) -> None:
        if not self.locked:
            try:
                # Subtract the position after anchoring adjustments to offset the detection to the proper place of the mask
                # simply subtracting self.pos will not work properly as self.pos is dynamic to the easing, while the mask is not
                if self.mask.get_at(inttup(pygame.mouse.get_pos() - anchored(self.anchor_pos, VEC(self.mask.get_size()), self.anchor))): # If the position of the cursor is part of the mask
                    self.hover()
                    if pygame.mouse.get_pressed()[0]:
    ...
    def generate_image(self, factor: float, swapped: bool) -> tuple[pygame.Surface, pygame.Mask]:
        ...
        p1, p2, p3, p4 = VEC(0, 0), VEC(size.x, 0), VEC(size.x, size.y), VEC(0, size.y)
